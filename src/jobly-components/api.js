...
class JoblyApi {
  ...
  static async applyToJob(id) {
    ...
    localStorage.setItem("userdata", JSON.stringify(mocks.user));
    return true;
  }

  /**gets list of jobs the user applied for */

  static async appliedJobList() {
    return mocks.user.applications;
  }
  ...
  static async login(data) {
    if (
      data.username === mocks.user.username &&
      data.password === mocks.user.password
    ) {
      localStorage.setItem("user", JSON.stringify(data.username));
      let storedData = JSON.parse(localStorage.getItem("userdata"));
      if (storedData) {
        this.saveProfile(storedData);
      } else {
        localStorage.setItem("userdata", JSON.stringify(mocks.user));
      }
      return mocks.token;
  ...
  static async signup(data) {
    ...
    let user = storedUser ? storedUser : mocks.user.username;
    if (data.username === user) validUser = false;
    ...
    if (validFields && validUser) {
      mocks.user = initialUser;
      await this.saveProfile(data);
      localStorage.clear();
      localStorage.setItem("user", JSON.stringify(data.username));
      localStorage.setItem("userdata", JSON.stringify(mocks.user));
  ...
  static async updateProfile(data) {
    ...
    return [validFields, validPassword];
  }
